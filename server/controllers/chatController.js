...
const parseChips = (text) => {
  ...
  const chipLineIndex = lines.findLastIndex((line) =>
    line.trim().startsWith("CHIPS:"),
  );
  if (chipLineIndex === -1)
    return { message: text.trim(), chips: defaultChips };
  ...
  try {
    ...
    return {
      ...
      chips:
        Array.isArray(chips) && chips.length ? chips.slice(0, 3) : defaultChips,
    };
  } catch {
    return {
      message: lines
        .filter((_, index) => index !== chipLineIndex)
        .join("\n")
        .trim(),
      chips: defaultChips,
    };
