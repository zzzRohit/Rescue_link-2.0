      ...
      <section className="grid min-h-[680px] items-center gap-14 py-16 lg:grid-cols-[minmax(0,1fr)_460px] lg:py-4">
        <div>
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600">
            RescueLink helps citizens report injured, stranded, or distressed
            animals, receive first-aid guidance, and connect with verified
            rescuers nearby.
          </p>
            ...
            <div className="p-6">
              <div className="mb-6">
                ...
                <h2 className="mt-2 text-2xl font-semibold text-slate-950">
                  Tell rescuers what happened
                </h2>
                <p className="mt-2 text-sm leading-6 text-slate-500">
                  Only the essentials needed for first response.
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <MockField
                  ...
                  value="Injured or unable to fly"
                />
                <MockField
                  label="Location"
                  value="Market Road, Bengaluru"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-xs text-slate-500">
                        Helps rescuers verify the case faster.
                      </p>
                    </div>
                  </div>
                </div>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {actions.map(({ to, title, text, Icon, primary }) => (
          ...
          >
            <div>
              <Icon
                className={`h-6 w-6 ${primary ? "text-green-50" : "text-green-700"}`}
              />
              <h3 className="mt-8 text-xl font-semibold">{title}</h3>
              <p
                className={`mt-3 text-sm leading-6 ${primary ? "text-green-50/85" : "text-slate-500"}`}
              >
                {text}
              </p>
            </div>
            <span
              className={`mt-6 inline-flex items-center gap-2 text-sm font-semibold ${primary ? "text-white" : "text-green-700"}`}
            >
              Open
      ...
      <section className="border-t border-slate-200 pt-16">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <h2 className="mt-3 text-3xl font-semibold text-slate-950">
              Choose what best matches the situation
            </h2>
          </div>
          <p className="max-w-md text-sm leading-6 text-slate-500">
            Categories keep reports consistent, so rescuers can understand
            urgency faster.
          </p>
        </div>
