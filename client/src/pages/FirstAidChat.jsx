...
export default function FirstAidChat() {
  ...
  const { messages, isLoading, send } = useChat({
    ...
    situationChips: selectedSituations,
  });
  ...
  const uploadImages = async (event) => {
    ...
    try {
      ...
      setUploadError(
        "Photo upload failed. Try a smaller JPG, PNG, or WebP image.",
      );
    }
  };
    ...
    <div className="flex gap-2 overflow-x-auto pb-2 md:grid md:grid-cols-2 md:overflow-visible md:pb-0">
      {animals.map((item) => (
        <button
          ...
          className={`shrink-0 rounded-full border px-3 py-1.5 text-xs ${animal === item ? "border-green-300 bg-green-50 text-green-800" : "border-gray-200 bg-white text-gray-600"}`}
        >
          {item}
        </button>
      ))}
  ...
  return (
    <div className="min-h-screen bg-[#F9FAF7] md:mx-auto md:flex md:h-[calc(100vh-137px)] md:max-w-6xl md:overflow-hidden md:rounded-2xl md:border md:border-gray-100 md:bg-white md:shadow-sm">
      <aside className="hidden w-[280px] shrink-0 border-r border-gray-100 bg-white p-5 md:flex md:flex-col">
        ...
        <p className="mt-4 rounded-full bg-green-50 px-3 py-2 text-xs text-green-800">
          Selected: {animal}
        </p>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {situations.map((situation) => (
            <button
              ...
              className={`rounded-full border px-3 py-1.5 text-xs ${selectedSituations.includes(situation) ? "border-green-300 bg-green-50 text-green-800" : "border-gray-200 text-gray-600"}`}
            >
              {situation}
            </button>
          ))}
        ...
        <div className="mt-auto space-y-3">
          <Link
            to="/rescuer"
            className="block rounded-xl border border-green-200 bg-green-50 px-4 py-3 text-center text-sm font-medium text-green-800"
          >
            Can't handle this?
          </Link>
          <p className="text-xs leading-5 text-gray-400">
            AI guidance only. Not a substitute for a veterinarian or trained
      ...
      <main className="flex h-full min-h-screen flex-1 flex-col md:min-h-0">
              ...
              >
                ...
                {message.streaming && (
                  <span className="ml-2 inline-flex gap-1 align-middle">
                    ...
                    <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-green-500 delay-100" />
                    <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-green-500 delay-200" />
                  </span>
                )}
              </div>
              {message.role === "assistant" && !message.streaming && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {(message.chips?.length ? message.chips : defaultChips).map(
                    (chip) => (
                      <button
                        key={chip}
                        type="button"
                        onClick={() => chipClick(chip)}
                        className="rounded-full border border-green-200 px-3 py-1.5 text-xs text-green-700 hover:bg-green-50"
                      >
                        {chip}
                      </button>
                    ),
        ...
        <div className="border-t border-gray-100 bg-white p-3">
          <p className="mb-3 rounded-lg border border-amber-100 bg-amber-50 px-3 py-2 text-xs text-amber-700">
            AI guidance only - contact a vet or trained animal rescuer for
            serious injuries
          </p>
          {images.length > 0 && (
              ...
              {images.map((src) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="absolute right-1 top-1 rounded-full bg-white p-1 text-gray-600 shadow"
                  >
                    <X className="h-3 w-3" />
                  </button>
                </div>
          ...
          <form onSubmit={submit} className="flex items-center gap-2">
            <button
              ...
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-600 hover:border-green-200 hover:bg-green-50 disabled:opacity-50"
            ...
            <input
              ...
              className="min-w-0 flex-1 rounded-full border border-gray-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-200"
            />
            <button
              disabled={
                (!input.trim() && !images.length) || isLoading || uploading
              }
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-green-600 text-white disabled:opacity-50"
            >
