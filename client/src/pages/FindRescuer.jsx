...
function FitMap({ rescuers }) {
  const map = useMap();
  useEffect(() => {
    const mappedRescuers = rescuers.filter(
      (rescuer) => Number.isFinite(rescuer.lat) && Number.isFinite(rescuer.lng),
    );
    if (!mappedRescuers.length) return;
    const bounds = L.latLngBounds(
      mappedRescuers.map((rescuer) => [rescuer.lat, rescuer.lng]),
    );
...
export default function FindRescuer() {
  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        setLocationDenied(true);
        setDetectedText(
          "Location access denied. Select your city to continue.",
        );
        setLocationLoading(false);
      },
  ...
  const visibleRescuers = useMemo(() => {
    const list =
      activeFilter === "24hr only"
        ? rescuers.filter((rescuer) => rescuer.available24hr)
        : rescuers;
    return [...list].sort(
      (a, b) =>
        Number(b.available24hr) - Number(a.available24hr) ||
        a.name.localeCompare(b.name),
    );
  }, [rescuers, activeFilter]);

  const mappedRescuers = visibleRescuers.filter(
    (rescuer) => Number.isFinite(rescuer.lat) && Number.isFinite(rescuer.lng),
  );
  const center = mappedRescuers[0]
    ? [mappedRescuers[0].lat, mappedRescuers[0].lng]
    : [12.9716, 77.5946];
    ...
    <div className="mx-auto max-w-5xl">
        ...
        {city ? (
            ...
            >
              ...
              {mappedRescuers.map((rescuer) => (
                <Marker
                  ...
                  icon={pin(rescuer.available24hr)}
                  eventHandlers={{ click: () => setSelectedId(rescuer._id) }}
                >
          ...
          <div className="flex h-[200px] flex-col items-center justify-center bg-[#F9FAF7] text-center md:h-[260px]">
            ...
            <p className="text-sm font-medium text-gray-700">
              Allow location or select your city above
            </p>
          </div>
        )}
      </section>
      ...
      <div className="mb-4 flex gap-2 overflow-x-auto pb-1">
        {filters.map((filter) => (
          <button
            ...
            className={`shrink-0 rounded-full border px-3 py-1.5 text-xs capitalize transition ${activeFilter === filter ? "border-green-300 bg-green-50 text-green-800" : "border-gray-200 bg-white text-gray-600 hover:bg-green-50"}`}
          >
            {filter}
          </button>
        ))}
      ...
      {city && !loading && visibleRescuers.length === 0 && (
        <div className="mb-4 rounded-2xl border border-amber-100 bg-amber-50 p-4 text-sm text-amber-700">
          No local rescue contacts found for this filter. Use the helplines
          below or try another category.
        </div>
      )}
      ...
      <div className="grid gap-3">
        {(loading ? [1, 2, 3] : visibleRescuers).map((item) =>
          loading ? (
            ...
            <article
              key={item._id}
              className={`rounded-2xl border bg-white p-4 shadow-sm transition ${selectedId === item._id ? "border-green-300 ring-2 ring-green-100" : "border-gray-100"}`}
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-400">
                    {item.address} ·{" "}
                    {item.available24hr ? "Open 24 hours" : "Day hours"}
                  </p>
                ...
                <span
                  className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${item.available24hr ? "bg-green-500" : "bg-gray-300"}`}
                />
              </div>
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          {nationalHelplines.map((helpline) => (
            <a
              ...
              className="rounded-xl border border-gray-100 p-3 text-sm hover:border-green-200 hover:bg-green-50"
            >
