...
const ProtectedRoute = ({ role, verified, children }) => {
  ...
  if (verified && !user.verified)
    return <Navigate to="/rescuer/pending-verification" replace />;
  return children;
};
  ...
  return (
    <Routes>
      <Route
        ...
        element={
          <ProtectedRoute role="rescuer" verified>
            <RescuerDashboard />
          </ProtectedRoute>
        }
      />
...
function Shell() {
  return (
    <>
      <Navbar />
      <main className="mx-auto min-h-[calc(100vh-137px)] max-w-6xl px-4 py-2 pb-24 sm:pb-8">
        <Routes>
          ...
          <Route
            path="/rescuer/pending-verification"
